...
The two evaluations were then re-run under the new rule rather than rescored in place, so each record and its captured log describe the same rule; on that new run the upper bound stands at 5 credits across six arms, all of them on `caprin-heterochromatin` and all of them earned on residue rather than on quotation.
Those 5 and the 1 above are two samples of the same six arms, not a before and an after: the like-for-like comparison is the rescoring, and it is the one that carries the argument.
