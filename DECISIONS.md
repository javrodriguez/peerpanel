...
(2) That strengthened assertion immediately earned its keep by catching a real defect of its own: the only planted-error subject at the time — `caprin-heterochromatin`, whose published twin is not a corpus member — is held out by construction, so nothing drops, and the check had to learn the difference between *nothing to exclude* and *exclusion failed*. (Read this as the subject it was written about, not as a property of the evaluation: `met17-auxotroph` was added later, its twin **is** a corpus member, and it drops 26 chunks on every arm. The records carry the difference in `twin_in_corpus`, and D25 states what that costs.)
...
**A third instance, and the sharpest of the three.** In the code that writes one arm's row of a
planted-error record, a local variable rebound the parameter carrying the index's dropped-chunk
count, so the number of quoted sentences the second stripping had removed was published as that
arm's exclusion figure. In the first regenerated records the three `caprin-heterochromatin` arms
read 6, 58 and 27 chunks dropped, while the same records said — correctly — that nothing had been
excluded: two answers to one question inside one file, on a subject that has no twin to withhold.
Those records were never committed, because `test_exclusion_state_cannot_contradict_itself` refused
them; that conformance test exists because an earlier round found two exclusion numbers that could
not disagree, and it earned its keep again here.
What it could not do is fail early. It reads a finished record, so it can only speak after a real
run, and this mistake was therefore found an hour of model time after it was made — the six arms of
the committed re-run sum to 64 minutes of model wall-clock between them.
The clause the rule was still missing: **when a check can only fail after an expensive run, add the
cheap twin that fails first.**
That twin is
`tests/test_planted.py::TestTheHarnessRunsThreeArms::test_the_exclusion_count_and_the_quotation_count_are_different_numbers`,
which drives the real harness on stub providers in under three seconds and discriminates by the property
rather than by the value: every arm searches the SAME index, so the exclusion count is identical
across arms, while the quotation count is a property of what each arm wrote and is not. It is
proven by mutation — restoring the rebinding turns it red where the shipped code is green, and the
mutated file was restored byte-identical afterwards.
...
**The rule this entry first left has been replaced, and it is worth saying which part of it was wrong.**
It read: "the committed ladders are measured with nothing else running, and are republished last, after every model run in a regeneration chain has finished; `latency_ms` and `run_utc` are declared run-varying in `evals.ablation.RUN_VARYING_FIELDS`, and the conformance test strips exactly those two before comparing a fresh ladder against the committed one".
The quiet-machine promise was not kept, and could not be checked: the two ladders committed here name their own conditions and read one-minute load averages of **18.49** (demo) and **19.43** (CI), which is not a machine with nothing else running.
That is this repository's own failure class — a claim about the conditions a number was produced under, made in prose, about a record with no field that could contradict it — standing in the entry whose whole subject is those conditions.
The answer that shipped is a field instead of a promise.
`AblationReport` carries `load_average_1m`, read as the run starts and declared run-varying beside `latency_ms` and `run_utc`, so a reader weighs the latency against the load it was taken under rather than trusting an assurance about a laptop; `results/RESULTS.md` states the same replacement, and this entry now agrees with it instead of contradicting it.
An idle laptop is not something a laptop can be held to; a recorded load is.

The declaration is three fields, not two: `RUN_VARYING_FIELDS = ("latency_ms", "run_utc", "load_average_1m")` in `evals.ablation`.
`tests/test_artifact_conformance.py` imports that constant rather than re-typing it, so it strips whatever the constant declares and no prose count of the fields — including this one — is ever the authority on it.
What survives unchanged is the reading rule: a precise multiple read off the latency column is a reading of one machine under one load rather than a property of the two rungs, so it is published with that condition attached and never as a bare ratio.
"Run-varying" describes which fields move between honest runs; it is not a licence to publish a number measured under load in silence, which is the specific silence the load field ends.
What the field does not yet get is a binding: no test in this repository reads `load_average_1m`, so the two load figures published from it are traceable to the records by hand and are not held to them by the suite.
...
Only one of those two generations is committed, so that number is attributed rather than recomputable: the two are the reports at `52238c8` and at `c607ff4`, and `git diff --name-only 52238c8 c607ff4 -- fixtures/summaries/demo` returns exactly one file. `results/RESULTS.md` states the same route beside the same figure.
That is what "the model layers reproduce the protocol, not the bytes" is worth as a number rather than as a hedge.

## D24 — Only a finding's own prose is scored, and that is now true rather than claimed
The round-3 fix dropped the `quote` FIELD from what a finding is scored on, and every page then said the quotation channel was closed.
Round 4 measured that it was not: nothing stopped a model putting manuscript text in the `text` field, and these models mostly do — **147 of the 235 scored strings in the records that round judged, 63%, were verbatim slices of the perturbed manuscript**, and every string that had ever earned a `named token` credit was one of them.
So the column published as an upper bound on NAMING was, in the records that carried it, measuring quotation.

Round 4 left two roads: say plainly that quotation still reaches the scored surface through `text`, or score only what is not manuscript.
The second is taken, because the first would leave a `named token` column that is almost entirely quotation — a column carrying no information about the arms — standing under a sentence explaining why.
`planted.own_prose(text, manuscript)` splits a finding on the shared abbreviation-aware boundary and drops any sentence whose whitespace-normalised, case-folded form is a substring of the same normalisation of the PERTURBED manuscript the arm was shown, before either rule sees what is left.
`DETECTION_RULE` moved from `assertion-v1` to `assertion-v2` because WHAT is scored changed; the cue alternation, the negation scope and the same-sentence clause did not move at all, and no cue was added, removed or reworded.

The property that makes this a fix rather than a tune: **stripping can only ever lower a count, never raise one.**
Both rules are existential over a list of strings — some sentence of some string satisfies them — and this removes text before they run, so anything credited on the residue would have been credited on the raw string too.
A post-hoc scoring change that is arithmetically incapable of flattering the headline is the one shape of post-hoc change that cannot have been fitted to its own result, which is why the input to the rule moved and the rule did not.
That property is proven rather than asserted: `tests/test_planted_soundness.py` runs both rules over every committed (error, string) pair, raw and stripped, and fails if one single credit appears that the raw string did not already carry.

The consequence, recorded in the direction it actually went.
The headline did not move: `detected` is 0 on every arm of both committed evaluations, before the change and after it.
The labelled upper bound fell — rescoring the records round 4 judged takes the `named token` count across their six arms from 6 credits to 1 — because five of those six were the arm quoting the perturbed sentence back rather than naming the token in a sentence of its own.
The two evaluations were then re-run under the new rule rather than rescored in place, so each record and its captured log describe the same rule.

**The upper bound stands at 3 credits across six arms, and the sentence this entry used to carry in its place was disproved by this entry's own repair.**
Until round 6 the paragraph above ended: "on that new run the upper bound stands at 5 credits across six arms, all of them on `caprin-heterochromatin` and all of them earned on residue rather than on quotation."
The first half went stale and the second half was never true.
The 5 was the count in the records committed at `192e804`; `C7.5` re-ran both evaluations and they carry **3** — one per arm on `caprin-heterochromatin`, none on `met17-auxotroph` — which is what the README's first screen and `results/RESULTS.md` publish.
"All of them earned on residue rather than on quotation" was an assertion with no measurement behind it, and round 5 measured it: `own_prose` compared a sentence to the manuscript without trimming the punctuation off its end, so a clause copied out of a paragraph and closed with a full stop was not a substring of it and survived into the residue.
Trim the ends and the bound falls from 5 credits to 3 on both record sets: on the `192e804` records this entry was describing, one credit comes off each single-agent arm on caprin; on the records committed here, both come off `qwen2:7b`, which goes 3/3 to 1/3.
Either way, two of the five were the perturbed manuscript quoted back verbatim with a full stop on the end — on these records *"Non-coding RNAs derived from pericentromeric repeat sequences are processed by Dicer (Dcr2) to generate short interfering (si)RNAs."* and the sentence naming the flipped direction.
So two of the five credits this entry called residue were exactly the quotation this entry exists to remove, and the rule it argues for is what proved it.
A count is not evidence for its own composition: the claim about what those credits were needed the same measurement the count did, and it did not have one.

Those 3 and the 1 above are two samples of the same six arms, not a before and an after: the like-for-like comparison is the rescoring, and it is the one that carries the argument.
The rescoring survives the repair — rerun over round 4's records with the punctuation tolerance in place, it still credits 1.
The rule's own limit stays stated wherever the number is: `own_prose` drops whole sentences only, so a sentence that copies the manuscript and adds three words of its own survives intact, and the count that survives it is an upper bound rather than a clean measure.

## D25 — One subject is held out, the other's twin is withheld at run time, and those are not the same guarantee
`caprin-heterochromatin` carries `twin_in_corpus: false`: its published twin was never a member of the demo corpus, so nothing has to be withheld for that evaluation and nothing is.
`met17-auxotroph` carries `twin_in_corpus: true`: its twin is `PMC10729969`, and every arm of that run drops the same 26 chunks and rebuilds the graph with the twin's extractions withheld.
Both facts are in both records, and a page that says "held-out manuscripts" of the pair generalises a property only one of them has.

Withholding is exact where retrieval reads chunks and approximate where it reads generated text, and the difference lands on one rung.
Chunk-level retrieval and `graphrag-local` carry no residue at all — the index is rebuilt without the twin's chunks and the graph is re-merged without its extractions.
Community reports are generated once over the whole corpus, so `graphrag-global` — the rung the panel's novelty reviewer retrieves with — ranks over text that carries the 101 twin-only entities `LIMITATIONS.md` measures, all 101 of them present in the `member_names` that rung tokenises.
The single-agent arms retrieve chunk-level RRF over BM25 and vectors and carry none of it.
So the asymmetry runs the panel's way: the arm that could be helped by the residue is the one this evaluation exists to test.

The bound belongs beside it rather than left for a reader to work out.
Contamination of that kind can only RAISE a detection count, and every detection count on both committed records is 0, on every arm, under both rules.
An advantage that could only have inflated a number cannot have produced a null, so the asymmetry is real, it is recorded with its direction and its size, and it does not reach the result.

## D26 — A count a filter could have produced is published with that filter's yield
`ReviewerOutput` carries `unretrieved_citations_dropped` and `malformed_findings_dropped`, both required and neither defaulted.
The first counts citation ids the hygiene pass removed from a finding because they name a chunk the reviewer was never given; the second counts whole findings it discarded before that list — not an object, no rubric dimension, or no text.
On both committed panel runs both counters are 0, on both reviewers.
That zero is what makes "of 16 findings in a run, 1 cites a retrieved chunk at all" a fact about the reviewers rather than a fact about the hygiene pass: without it, a reviewer that cited nothing and a reviewer whose citations this pass deleted read identically in the record.
The argument is the one this repository already made for `unparsed_calls` on an evaluation arm — a call that produced nothing silently lowers a detection count, so the number of them is published beside the count it lowered — and the fields are required for D18's reason: a 0 written by a default is the same untellable number wearing a measurement's clothes.

There is a neighbouring case these counters deliberately do NOT absorb, and it is stated here because the same reader will ask it next.
Both reviewers returned exactly `MAX_FINDINGS` — 8 — on both committed runs, so "16 findings in a run" is twice a cap rather than the reviewers' own output, and any per-finding rate over it has that cap in its denominator.
The cap is answerable for that, not the hygiene pass: the loop stops at `MAX_FINDINGS`, so nothing past it is examined and neither counter could honestly count it, and the record shows the state plainly by carrying a `findings` list whose length is the constant in the code.
Counting what the cap removed would mean counting output a reviewer's own schema told it not to write, which is a different measurement and would need a different run.
