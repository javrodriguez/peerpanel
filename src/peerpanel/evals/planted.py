...
def plant_errors(text: str, source_file: str, n_per_kind: int = 1) -> PlantedManuscript:
    ...
    pvalues = [(i, m.group(0)) for i, line in enumerate(lines) for m in [_PVALUE.search(line)] if m]
    ...
    body = [i for i, line in enumerate(lines) if len(line.split()) > 25]
    ...
    percents = [
        (i, m.group(0)) for i, line in enumerate(lines) for m in [_PERCENT.search(line)] if m
    ]
    for i, pct in rng.sample(percents, min(n_per_kind, len(percents))):
        _apply(i, pct, "412%", "impossible_percentage", "412%")

    return PlantedManuscript(source_file=source_file, text="\n".join(lines), errors=errors)
